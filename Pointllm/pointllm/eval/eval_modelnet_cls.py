...
from pointllm.data import ShapeNet
from pointllm.data import ScanObjectNN
from pointllm.data import ScanObjectNN_train
from pointllm.data import S3DIS
...
def load_dataset(config_path, split, subset_nums, use_color, dataset_name):
    print(f"Loading {split} split of {dataset_name} dataset.")

    if dataset_name == "ModelNet":
        dataset = ModelNet(config_path=config_path, split=split,
                           subset_nums=subset_nums, use_color=use_color)

    elif dataset_name == "ShapeNet":
        dataset = ShapeNet(config_path=config_path, split=split,
                           subset_nums=subset_nums, use_color=use_color)
    elif dataset_name == "S3DIS":
        dataset = S3DIS(config_path=config_path, split=split,
                               subset_nums=subset_nums, use_color=use_color)
    else:
...
def start_generation(model, tokenizer, conv, dataloader, prompt_index, output_dir, output_file, dataset_name):
    ...
    responses = []
    if dataset_name == "ModelNet":
        with open('concat_f_values_MN.txt', 'w') as f:
            for batch in tqdm(dataloader):
                point_clouds = batch["point_clouds"].cuda().to(model.dtype)
                model.eval()
                point_features = model.get_model().point_backbone(point_clouds)
                concat_f = torch.cat([point_features[:, 0], point_features[:, 1:].max(1)[0]], dim=-1)

                concat_f_np = concat_f.cpu().detach().float().numpy()  
                f.write(str(concat_f_np) + '\n')
    elif dataset_name == "ShapeNet":
        with open('concat_f_values_SN.txt', 'w') as f:
            for batch in tqdm(dataloader):
                point_clouds = batch["point_clouds"].cuda().to(model.dtype)
                model.eval()
                point_features = model.get_model().point_backbone(point_clouds)
        ...
        with open('concat_f_values_SD.txt', 'w') as f:
            for batch in tqdm(dataloader):
                point_clouds = batch["point_clouds"].cuda().to(model.dtype)
                model.eval()
                point_features = model.get_model().point_backbone(point_clouds)
...
def main(args):
    ...
    # * output file 
    args.output_file = f"{args.dataset_name}_classification_prompt{args.prompt_index}.json"
    ...
    if not os.path.exists(args.output_file_path):
        ...
        dataset = load_dataset(
            ...
            dataset_name=args.dataset_name  # 新增
        )
        ...
        results = start_generation(model, tokenizer, conv, dataloader, args.prompt_index, args.output_dir, args.output_file, args.dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default="ModelNet",
                        choices=["ModelNet", "ScanObjectNN", "ShapeNet", "S3DIS"],
