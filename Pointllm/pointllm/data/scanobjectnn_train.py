...
class ScanObjectNN_train(Dataset):
    def __init__(self, config_path, split, subset_nums=-1, use_color=False):
        ...
        Args:
            data_args:
                split: train or test
        """
        super(ScanObjectNN_train, self).__init__()

        if config_path is None:
            # * use the default config file in the same dir
            config_path = os.path.join(os.path.dirname(__file__), "scanobjectnn_train_config", "ScanObjectNN_train15.yaml")
        ...
        self.catfile = os.path.join(os.path.dirname(__file__), "scanobjectnn_train_config", 'scanobjectnn_train15_shape_names_modified.txt')

        # "tv_stand" -> "tv stand"
        self.categories = [line.rstrip() for line in open(self.catfile)] # * list of category names

        self.save_path = os.path.join(self.root,
                                    'scanobjectnn_train%d_%s_%dpts_fps.dat' % (self.num_category, self.split, self.npoints))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ShapeNet Dataset')
