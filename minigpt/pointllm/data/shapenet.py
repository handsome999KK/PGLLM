...
class ShapeNet(Dataset):
    def __init__(self, config_path, split, subset_nums=-1, use_color=False):
        ...
        if config_path is None:
            ...
            config_path = os.path.join(os.path.dirname(__file__), "shapenet_config", "ShapeNet54.yaml")
        ...
        self.catfile = os.path.join(os.path.dirname(__file__), "shapenet_config", 'shapenet54_shape_names_modified.txt')

        # "tv_stand" -> "tv stand"
        self.categories = [line.rstrip() for line in open(self.catfile)] # * list of category names

        self.save_path = os.path.join(self.root,
                                    'shapenet%d_%s_%dpts_fps.dat' % (self.num_category, self.split, self.npoints))
