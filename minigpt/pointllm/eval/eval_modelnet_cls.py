...
# def load_dataset(data_path, config_path, split, subset_nums, use_color, dataset_name):
#     if dataset_name == "ModelNet":
#         dataset = ModelNet(config_path=config_path, split=split, subset_nums=subset_nums, use_color=use_color)
#     elif dataset_name == "ShapeNet":
#         dataset = ShapeNet(config_path=config_path, split=split, subset_nums=subset_nums, use_color=use_color)
#     return dataset
...
def start_generation(model,  dataloader, prompt_index, output_dir, output_file, args):
    with open('minigpt_features_MN.txt', 'w') as f:
        for batch in tqdm(dataloader):
            point_clouds = batch["point_clouds"].cuda()
            model.eval()

            outputs = model.encode_pc(point_clouds)
            point_features, _ = outputs

            concat_f = torch.cat([
                point_features[:, 0],
                point_features[:, 1:].max(1)[0]
            ], dim=-1)

            concat_f_np = concat_f.cpu().detach().float().numpy()
            np.savetxt(f, concat_f_np, fmt='%.6f')
...
def main(args):
    ...
    if not os.path.exists(args.output_file_path):
        ...
        # dataset = load_dataset( data_path=args.data_path, config_path=None, split=args.split, subset_nums=args.subset_nums, use_color=args.use_color, dataset_name=args.dataset_name, )
...
if __name__ == "__main__":
    ...
    #                     choices=["ModelNet", "ScanObjectNN", "ShapeNet", "ScanObjectNN_train", "S3DIS"],
